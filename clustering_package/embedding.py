...
class STEmbedding:
    ...
    def text2embedding(self, texts):
        ...
        def mean_pooling(model_output_cur, attention_mask):
            token_embeddings = model_output_cur[0]  # First element of model_output contains all token embeddings
    ...
    def load_embedding(self, dataset_name):
        return torch.load(os.path.join(self.save_path, f'dataset_{dataset_name}/embedding_{dataset_name}.tz'))

    def save_embedding(self, dataset_name, documents_embedding):
        utils.path_check(os.path.join(self.save_path, f'dataset_{dataset_name}/'), if_create=True)
        torch.save(documents_embedding, os.path.join(self.save_path,
...
class STEmbeddingLang:
    ...
    def load_embedding(self, dataset_name, language):
        ...
        dataset_embedding = utils.read_data(os.path.join(
            self.save_path, f'dataset_{dataset_name}/embedding_{language}.npy'), data_type='np')
    ...
    def save_embedding(self, dataset_name, document_embeddings, document_ids, language):
        ...
        utils.save_data(document_embeddings.detach().numpy(),
                        os.path.join(self.save_path, f'dataset_{dataset_name}/embedding_{language}.npy'),
                        data_type='np')
        utils.save_data(document_ids, os.path.join(self.save_path, f'dataset_{dataset_name}/id_{language}.json'))

    def compute(self, dataset_name, dataset_source):
        n = (len(dataset_source['id_order'][dataset_source['language_1']]) +
             len(dataset_source['id_order'][dataset_source['language_2']]))
...
class AEEmbedding:
    ...
    def run_ae(self, dataset_name, seed, **kwargs):
        ...
        if pre_ae_arg is None:
            ...
            ae_ver, pre_ae_arg = run.run_util.version_verification('ae', alg_info=pre_ae_arg,
                                                                   input_version=self.version, mode_='create',
                                                                   verbose_level=kwargs.get('verbose_level', 1))
            self.version = ae_ver
