...
def run_autoencoder(dataset_name, seed, **kwargs):
    ...
    model_save_path = os.path.join(save_path, 'models/AE/model/')
    ...
    model = models.AutoEncoder(input_size=dataset.embedding_size, embedding_dim=ae_arg['embedding_dim'],
                               ...
                               dist_metric=ae_arg['dist_metric'], random_state=seed, hidden_1_size=ae_arg['hidden_1_size'],
    ...
    earlystopper = run_util.EarlyStopper(tolerance=ae_arg['tolerance'], delta=ae_arg['delta'], off=ae_arg['early_stop'])
